...
for fidx in range(len(fits)):
  ...
  if _fit.split(":")[0] == "fixed":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit -m 125.38 %s --floatOtherPOIs 1 %s -n _%s_%s -P %s --algo fixed %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  if _fit.split(":")[0] == "AsymptoticLimit":
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M AsymptoticLimits -m 125.38 %s %s -n _%s_%s --redefineSignalPOI %s %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_fit_opts,pdf_opts,common_opts)
      else:
  ...
  # For 1D scan when fixing other pois
    ...
    for poi in _fitpois:
      if opt.batch == 'local':
        fitcmd = "combine -M MultiDimFit -m 125.38 %s --floatOtherPOIs 0 %s -n _%s_%s -P %s --algo grid --points %s --alignEdges 1 %s %s %s"%(d_opts,exp_opts,_name,poi,poi,_points.split(":")[0],_fit_opts,pdf_opts,common_opts)
      else:
  ...
  # For 2D scan: fix other pois to 0
    ...
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125.38 %s -P %s -P %s --floatOtherPOIs 1 %s -n _%s_%s --algo grid --points %s --alignEdges 1 --split-points %s %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_points.split(":")[1],_fit_opts,pdf_opts,common_opts)
    else:
  ...
  # For 2D scan: fix other pois to 0
    ...
    _poisStr = "%s_vs_%s"%(_fitpois[0],_fitpois[1])
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125.38 %s -P %s -P %s --floatOtherPOIs 0 %s -n _%s_%s --algo grid --points %s --alignEdges 1 --split-points %s %s %s %s"%(d_opts,_fitpois[0],_fitpois[1],exp_opts,_name,_poisStr,_points.split(":")[0],_points.split(":")[1],_fit_opts,pdf_opts,common_opts)
    else:
  ...
  # Robust Hesse
    ...
    _poi = _fitpois[0]
    if opt.batch == 'local':
      fitcmd = "combine -M MultiDimFit -m 125.38 %s -P %s --floatOtherPOIs 1 %s -n _%s --robustHesse 1 --robustHesseSave 1 --saveFitResult %s %s %s"%(d_opts,_poi,exp_opts,_name,_fit_opts,pdf_opts,common_opts)
    else:
